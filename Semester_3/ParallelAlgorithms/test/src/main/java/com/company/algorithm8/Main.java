...
public class Main {
    ...
    public static void main(String[] args) {
        setup(8, 4, new PairType[]{new PairType(1, 2), new PairType(0, 3), new PairType(2, 4), new PairType(1, 5)});
    ...
    public static void launch(int numOfThreads, int numOfPairs, PairType[] input) {
        setup(numOfThreads, numOfPairs, input);
        int interval = pairsAmount / threadsAmount;
        Thread[] threads = new Thread[threadsAmount];
        for (int i = 0; i < threadsAmount; i++) {
            threads[i] = new Thread(new Calculator(i * interval, Math.min((i + 1) * interval, pairsAmount), i, pairs));
